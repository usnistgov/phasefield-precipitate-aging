...
def labelAxes(xlabel, ylabel, n):
  ...
  def plot_ticks(start, stop, tick, angle, n):
    ...
    plt.text(simX(-0.11, 0.55), simY(0.55), ylabel, rotation=60, fontsize=18, color=grays[dark])
    ...
    if angle >= 0:
      for i in range(len(x)):
        plt.text(x[i] + dx , y[i] + dy, "{0:.1f}".format(r[i]), rotation=angle, color=grays[dark], **alignment)
    else:
      midx = 0.5 * (start[0] + stop[0])
      midy = 0.5 * (start[1] + stop[1])
      plt.text(midx + dx, midy + dy, "0.5", rotation=angle, color=grays[dark], **alignment)
...
plt.text(simX(0.050, 0.450), simY(0.450), '$\gamma$',  color='black', alpha=0.1, fontsize=20, **alignment)
plt.text(simX(0.235, 0.050), simY(0.050), '$\delta$',  color='black', alpha=0.1, fontsize=20, **alignment)
plt.text(simX(0.280, 0.365), simY(0.365), '$\lambda$', color='black', alpha=0.1, fontsize=20, **alignment)
...
plt.savefig("ternary-landscape.png", dpi=400, bbox_inches="tight",
