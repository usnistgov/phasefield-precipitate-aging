...
double inv_fict_det(double f_del, double f_gam, double f_lav) {
   ...
   inv_fict_det_result = 1.0/(1.15317496889714*pow(f_del, 2) + 42.9640255033086*f_del*f_gam + 4.35012289609114*f_del*f_lav + 102.885881033764*pow(f_gam, 2) + 51.3939292669239*f_gam*f_lav + 2.32367001744861*pow(f_lav, 2));
...
double fict_gam_Cr(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_gam_Cr_result = 1.0*INV_DET*(36.9670932572906*XCR*f_del + 102.885881033764*XCR*f_gam + 26.3010348077371*XCR*f_lav - 14.4030540553513*XNB*f_del - 78.3270137345168*XNB*f_lav + 3.53929757946133*pow(f_del, 2) + 1.75528493642612*f_del*f_gam + 14.5612368148933*f_del*f_lav - 16.6120706817648*f_gam*f_lav + 18.9395483711502*pow(f_lav, 2));
...
double fict_gam_Nb(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_gam_Nb_result = -1.0*INV_DET*(7.15429731355127*XCR*f_del + 5.37357718050628*XCR*f_lav - 5.99693224601796*XNB*f_del - 102.885881033764*XNB*f_gam - 25.0928944591868*XNB*f_lav + 1.33310456634871*pow(f_del, 2) + 20.7772578381999*f_del*f_gam + 5.77910699501942*f_del*f_lav + 30.4559902904332*f_gam*f_lav + 6.56030447802516*pow(f_lav, 2));
...
double fict_del_Cr(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_del_Cr_result = 1.0*INV_DET*(1.15317496889714*XCR*f_del + 5.99693224601796*XCR*f_gam + 0.780764088654332*XCR*f_lav + 14.4030540553513*XNB*f_gam - 1.05269525818007*XNB*f_lav - 3.53929757946133*f_del*f_gam - 0.0345017703034707*f_del*f_lav - 1.75528493642612*pow(f_gam, 2) - 6.10862948694289*f_gam*f_lav + 0.145909868348492*pow(f_lav, 2));
...
double fict_del_Nb(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_del_Nb_result = 1.0*INV_DET*(7.15429731355127*XCR*f_gam - 0.101861460320036*XCR*f_lav + 1.15317496889714*XNB*f_del + 36.9670932572906*XNB*f_gam + 3.56935880743681*XNB*f_lav + 1.33310456634871*f_del*f_gam - 0.185426266173148*f_del*f_lav + 20.7772578381999*pow(f_gam, 2) - 1.71930496668943*f_gam*f_lav - 0.57089209646835*pow(f_lav, 2));
...
double fict_lav_Cr(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_lav_Cr_result = 1.0*INV_DET*(3.56935880743681*XCR*f_del + 25.0928944591868*XCR*f_gam + 2.32367001744861*XCR*f_lav + 1.05269525818007*XNB*f_del + 78.3270137345168*XNB*f_gam + 0.0345017703034707*pow(f_del, 2) - 8.45260732795046*f_del*f_gam - 0.145909868348492*f_del*f_lav + 16.6120706817648*pow(f_gam, 2) - 18.9395483711502*f_gam*f_lav);
...
double fict_lav_Nb(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_lav_Nb_result = 1.0*INV_DET*(0.101861460320036*XCR*f_del + 5.37357718050628*XCR*f_gam + 0.780764088654332*XNB*f_del + 26.3010348077371*XNB*f_gam + 2.32367001744861*XNB*f_lav + 0.185426266173148*pow(f_del, 2) + 7.49841196170884*f_del*f_gam + 0.57089209646835*f_del*f_lav + 30.4559902904332*pow(f_gam, 2) + 6.56030447802516*f_gam*f_lav);
