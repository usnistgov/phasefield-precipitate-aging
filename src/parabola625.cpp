...
double inv_fict_det(double f_del, double f_gam, double f_lav) {
   ...
   inv_fict_det_result = 0.060746916264914186/(0.011208291723902289*pow(f_del, 2) + 0.41758912954449995*f_del*f_gam + 0.04228104820974949*f_del*f_lav + 1.0*pow(f_gam, 2) + 0.49952363483243967*f_gam*f_lav + 0.022584926076358817*pow(f_lav, 2));
...
double fict_gam_Cr(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_gam_Cr_result = 16.461740965402281*INV_DET*(0.35930190698526532*XCR*f_del + 1.0*XCR*f_gam + 0.25563308146339203*XCR*f_lav - 0.13999057898551379*XNB*f_del - 0.76129992713783667*XNB*f_lav + 0.034400226191384116*pow(f_del, 2) + 0.017060503528662844*f_del*f_gam + 0.14152803736126604*f_del*f_lav - 0.16146113067071988*f_gam*f_lav + 0.18408306543960862*pow(f_lav, 2));
...
double fict_lav_Cr(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_lav_Cr_result = 12.532322197522699*INV_DET*(0.045569959037821371*XCR*f_del + 0.32036066821386966*XCR*f_gam + 0.029666265910819816*XCR*f_lav + 0.013439747131788969*XNB*f_del + 1.0*XNB*f_gam + 0.00044048366787493193*pow(f_del, 2) - 0.10791433153062506*f_del*f_gam - 0.0018628294555316793*f_del*f_lav + 0.21208609762744221*pow(f_gam, 2) - 0.24180097603802814*f_gam*f_lav);
...
double g_del(double XCR, double XNB) {
   ...
   double g_del_result;
   g_del_result = 32328981887.7551*pow(XCR - 0.014999999999999999, 2) + (16970167763.733316*XCR - 254552516.45599973)*(XNB - 0.245) + 101815797663.26523*pow(XNB - 0.245, 2);
   return g_del_result;
