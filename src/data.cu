               ...
               struct CudaData* dev)
{
	...
	checkCuda(cudaMalloc((void**) &(dev->mob_phi_del_Cr), nx * ny * sizeof(fp_t)));
	checkCuda(cudaMalloc((void**) &(dev->mob_phi_del_Nb), nx * ny * sizeof(fp_t)));
...
void free_cuda(struct CudaData* dev)
{
	...
	*/
	...
	checkCuda(cudaFree(dev->mob_phi_del_Cr));
