...
__device__ double d_xr_gam_Nb(double P_del, double P_lav) {
   ...
   xr_gam_Nb_result = 1.914953946717973e-10*P_del - 7.5073446491249387e-11*P_lav + 0.018;
...
__device__ double d_xr_del_Nb(double P_del, double P_lav) {
   ...
   xr_del_Nb_result = -3.0098560476695523e-12*P_del + 2.8083066899508087e-11*P_lav + 0.2440;
...
__device__ double d_xr_lav_Nb(double P_del, double P_lav) {
   ...
   xr_lav_Nb_result = 1.2446553532108367e-11*P_del + 1.3771353705435363e-11*P_lav + 0.2590;
...
__device__ double d_inv_fict_det(double f_del, double f_gam, double f_lav) {
   ...
   inv_fict_det_result = 1.1017502786047317/(0.028198882496280081*f_del*f_del + 0.58678637752223428*f_del*f_gam + 0.094379501815769914*f_del*f_lav + 1.0*f_gam*f_gam + 0.60304745699564377*f_gam*f_lav + 0.061011302923507621*f_lav*f_lav);
...
__device__ double d_fict_gam_Cr(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_gam_Cr_result = 0.90764669582512891*INV_DET*(0.50514638806645196*XCR*f_del + 1.0*XCR*f_gam + 0.42609155984759101*XCR*f_lav - 0.16577335655420122*XNB*f_del - 0.26381641966842839*XNB*f_lav + 0.04222033549765767*f_del*f_del + 0.020044914882261487*f_del*f_gam - 0.043567313569073896*f_del*f_lav - 0.27734945844324121*f_gam*f_lav - 0.059424948213882431*f_lav*f_lav);
...
__device__ double d_fict_lav_Nb(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_lav_Nb_result = -0.38674059641464115*INV_DET*(0.01401212615850367*XCR*f_del - 0.12799677738478332*XCR*f_gam - 0.060421534040585929*XNB*f_del - 1.0*XNB*f_gam - 0.14318824560930238*XNB*f_lav - 0.0027593655114734393*f_del*f_del - 0.11564489921674229*f_del*f_gam - 0.01203607258738176*f_del*f_lav - 0.52265226768031503*f_gam*f_gam - 0.056986564033684621*f_gam*f_lav);
...
__device__ double d_g_gam(double XCR, double XNB) {
   ...
   double g_gam_result;
   g_gam_result = 2225414588.9007587*pow(XCR - 0.525, 2) + (14903715603.987373*XCR - 7824450692.0933714)*(XNB - 0.018) + 44805620198.983856*pow(XNB - 0.018, 2);
   return g_gam_result;
...
__device__ double d_g_del(double XCR, double XNB) {
   ...
   double g_del_result;
   g_del_result = 19076223392.853703*(XCR - 0.0258)*(XCR - 0.0258) + (16983307497.06665*XCR - 438169333.42431957)*(XNB - 0.2440) + 85911591727.909775*pow(XNB - 0.2440, 2);
   return g_del_result;
...
__device__ double d_g_lav(double XCR, double XNB) {
   ...
   double g_lav_result;
   g_lav_result = 8880622842.3634224*(XCR - 0.375)*(XCR - 0.375) + (23980778596.615067*XCR - 8992791973.7306499)*(XNB - 0.2590) + 97731010299.215485*pow(XNB - 0.2590, 2);
   return g_lav_result;
