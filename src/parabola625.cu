...
__device__ double d_inv_fict_det(double f_del, double f_gam, double f_lav) {
   ...
   inv_fict_det_result = 1.0/(0.0255946225237097*f_del*f_del + 0.532594716713253*f_del*f_gam + 0.0856632429767053*f_del*f_lav + 0.907646695825129*f_gam*f_gam + 0.547354031767842*f_gam*f_lav + 0.0553767075065077*f_lav*f_lav);
...
__device__ double d_fict_gam_Cr(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_gam_Cr_result = 1.0*INV_DET*(0.458494450036513*XCR*f_del + 0.907646695825129*XCR*f_gam + 0.386740596414641*XCR*f_lav - 0.150463639332262*XNB*f_del - 0.239452101616465*XNB*f_lav + 0.0383211480110774*f_del*f_del + 0.0181937007609806*f_del*f_gam - 0.0395437282069472*f_del*f_lav - 0.251735319544897*f_gam*f_lav - 0.0539368578959098*f_lav*f_lav);
...
__device__ double d_fict_lav_Nb(double INV_DET, double XCR, double XNB, double f_del, double f_gam, double f_lav) {
   ...
   fict_lav_Nb_result = -1.0*INV_DET*(0.0054190580275769*XCR*f_del - 0.0495015500249432*XCR*f_gam - 0.0233674601111437*XNB*f_del - 0.386740596414641*XNB*f_gam - 0.0553767075065077*XNB*f_lav - 0.00106715866363323*f_del*f_del - 0.044724577295394*f_del*f_gam - 0.00465483789093393*f_del*f_lav - 0.20213084972015*f_gam*f_gam - 0.0220390177620083*f_gam*f_lav);
...
__device__ double d_g_gam(double XCR, double XNB) {
   ...
   double g_gam_result;
   g_gam_result = 2225414588.90076*(XCR - 0.525)*(XCR - 0.525) + (14903715603.9874*XCR - 7824450692.09337)*(XNB - 0.018) + 44805620198.9839*(XNB - 0.018)*(XNB - 0.018);
   return g_gam_result;
...
__device__ double d_g_del(double XCR, double XNB) {
   ...
   double g_del_result;
   g_del_result = 19076223392.8537*(XCR - 0.0258)*(XCR - 0.0258) + (16983307497.0667*XCR - 438169333.42432)*(XNB - 0.244) + 85911591727.9098*(XNB - 0.244)*(XNB - 0.244);
   return g_del_result;
...
__device__ double d_g_lav(double XCR, double XNB) {
   ...
   double g_lav_result;
   g_lav_result = 8880622842.36342*(XCR - 0.375)*(XCR - 0.375) + (23980778596.6151*XCR - 8992791973.73065)*(XNB - 0.259) + 97731010299.2155*(XNB - 0.259)*(XNB - 0.259);
   return g_lav_result;
