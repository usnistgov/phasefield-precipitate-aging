    ...
    if path.isdir(datdir) and len(glob.glob("{0}/*.xy".format(datdir))) > 0:
        ...
        for file in fnames[::10]:
            try:
                ...
                plt.plot(simX(xnb, xcr), simY(xcr), '-', markersize=2, linewidth=1, zorder=1, color='gray')
        ...
        plt.savefig("diagrams/TKR4p149/pathways_zoom_{0}.png".format(base), dpi=400, bbox_inches='tight')
        plt.close()
    else:
