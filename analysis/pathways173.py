...
for datdir in glob.glob('/usr/local/tnk10/data/phase-field/alloy625/TKR4p173/run*'): #{0}".format(j) for j in (2,3,6,12,21,63)]:
    base = path.basename(datdir)
    if path.isdir(datdir) \
    and len(glob.glob("{0}/../../coord/TKR4p173/{1}/*.xy".format(datdir, base))) > 0 \
    and len(glob.glob("{0}/../../coord/TKR4p173/diffusion_{1}.xc".format(datdir, base))) > 0:
        # Plot phase diagram
        ...
        fnames = sorted(glob.glob("{0}/../../coord/TKR4p173/{1}/*.xy".format(datdir, base)))
        ...
        try:
            dgcr, dgnb, dcr, dnb, lgcr, lgnb, lcr, lnb = np.loadtxt("{0}/../../coord/TKR4p173/diffusion_{1}.xc".format(datdir, base), delimiter=',', skiprows=1, usecols=(3,4,5,6,11,12,13,14), unpack=True)
            ...
            print("Empty file: {0}/../../coord/TKR4p173/diffusion_{1}.xc".format(datdir, base))

        plt.xlim([0, 0.6])
        plt.ylim([0, rt3by2*0.6])
        plt.legend(loc='best')
        plt.savefig("diagrams/TKR4p173/pathways/pathways_{0}.png".format(base), dpi=400, bbox_inches='tight')
        ...
        plt.savefig("diagrams/TKR4p173/pathways/pathways_zm_gam_{0}.png".format(base), dpi=400, bbox_inches='tight')
        ...
        plt.savefig("diagrams/TKR4p173/pathways/pathways_zm_del_{0}.png".format(base), dpi=400, bbox_inches='tight')
        ...
        plt.savefig("diagrams/TKR4p173/pathways/pathways_zm_lav_{0}.png".format(base), dpi=400, bbox_inches='tight')
        ...
        plt.xlim([0, 0.6])
        plt.ylim([0, rt3by2*0.6])
        plt.savefig("diagrams/TKR4p173/triangles/composition_{0}.png".format(base), dpi=300, bbox_inches='tight')
        plt.close()
    else:
