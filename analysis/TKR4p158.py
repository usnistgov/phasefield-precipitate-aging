...
fmax = 0.0
datasets = []
for datdir in tqdm(glob.glob("/data/tnk10/phase-field/alloy625/TKR4p158/run*/")):
...
summary = open("analysis/TKR4p158-summary.csv", "w")
...
for base, xCr, xNb, fd, fl in tqdm(datasets):
    ...
    plt.legend(loc="best")
    plt.savefig(
        "diagrams/TKR4p158/ratios/ratio_{0}.png".format(base),
        dpi=400,
        bbox_inches="tight",
    )
    plt.close()
    ...
    plt.legend(loc="best")
    plt.savefig(
        "diagrams/TKR4p158/phases/phase_{0}.png".format(base),
        dpi=400,
        bbox_inches="tight",
    )
    plt.close()
