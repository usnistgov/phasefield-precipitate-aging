...
# Usage: python analysis/TKR4p173.py
...
def twoPhaseDemarcation(x):
    m = (xe_gam_Nb - xe_del_Nb) / (xe_gam_Cr - xe_del_Cr)
    b = xe_del_Nb
...
for datdir in tqdm(glob.glob("/usr/local/tnk10/data/phase-field/alloy625/TKR4p173/run*")):
    if path.isdir(datdir) and len(glob.glob("{0}/c.log".format(datdir))) > 0:
        ...
        try:
            ...
            aCr = leverCr(xNb[0], xCr[0], dx_del_Nb, dx_del_Cr, dx_lav_Nb, dx_lav_Cr, dx_gam_Nb, dx_gam_Cr)
            aNb = leverNb(xNb[0], xCr[0], dx_del_Nb, dx_del_Cr, dx_lav_Nb, dx_lav_Cr, dx_gam_Nb, dx_gam_Cr)
            fd0 = np.sqrt(((aNb - xNb[0])**2 + (aCr - xCr[0])**2) / ((aNb - dx_del_Nb)**2 + (aCr - dx_del_Cr)**2))
            ...
            datmax = max(np.amax(fl0), np.amax(fd0))
            if datmax > fmax:
                fmax = 1.005 * datmax
            """
        except:
